...
Job ad + recruiting quiz for **MÖBEL MARTIN** (Küchenplaner / Küchenberater, Konz): React 18 + TypeScript +
Tailwind + shadcn/ui, bundled by Vite as a multi-page app. Forked from the my-next-job quiz engine, which is
config-driven — the quiz lives in `quizzes/kuechenplaner-konz.json`, and its questions, scoring, knockouts and
branding are data, not code. Deployed on Vercel
...
matching the live page at my-next-job.de/kuechenplaner-m-w-d/. See `README.md`, `product.md`, `design.md` for
detail; this file covers only what a coding agent needs.

**Client assets are missing.** MÖBEL MARTIN has supplied no logo file and no photography, so:
`company.logo` is `''` and the header/footer render `Wordmark.tsx` (type-set name) instead; the hero's image
column falls back to a branded gradient panel; the team section is removed from `LandingPage`. Drop real files
into `assets/`, set `company.logo` / `hero.photo`, and both fall back to the image automatically.
The imprint and privacy pages stay PPC GmbH / my-next-job — the agency operates the page, MÖBEL MARTIN is the
advertised employer.
...
app.html                # quiz engine — mounts src/entries/quiz.tsx, reads ?q=kuechenplaner-konz
impressum.html          # legal pages, same pattern
datenschutz.html
src/
  entries/*.tsx         # one thin mount per HTML page
  pages/Landing/        # job-ad sections; all copy lives in content.ts (Sie-Form)
  pages/Quiz/
...
quizzes/kuechenplaner-konz.json  # the client's quiz: 12 questions, Sie-Form
assets/                 # favicons only so far — no client logo or photography yet
...
- All user-visible strings are German (`lang="de"`) and use the **Sie-Form** — this client's audience is
  addressed formally, unlike the PPC ad this was forked from. Landing copy belongs in
